...
test('/api/usage payload carries rhythm, mode, provider and a previous-window projection', async () => {
  ...
  try {
    ...
    assert.equal(Object.hasOwn(payload, 'prompts'), false,
      'the retired Prompts projection is not part of the usage payload');
...
test('the retired Prompts view is absent while its legacy bookmark lands on Scorecard', () => {
  assert.match(JS, /VIEWS=\["score","limits","findings","models","sessions","transcript"\]/,
    'the supported usage navigation must not advertise the retired view');
  assert.match(JS, /parts\[1\]==="prompts"\)\{usageView="score";/,
    'an old #usage/prompts bookmark must land on Scorecard rather than become a transcript id');
});
