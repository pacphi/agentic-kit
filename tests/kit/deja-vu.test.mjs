...
test('command construction uses one explicit target and suppresses guidance and per-target warmup', () => {
  ...
  assert.deepEqual(buildDejaVuUninstallCommand('opencode', 'auto'), {
    command: 'deja', args: ['uninstall', 'opencode-auto', '--no-guidance', '--no-index'],
  });
  for (const bad of [
...
test('direct target observation is content-free and distinguishes MCP from auto wiring', () => {
  ...
  try {
    ...
    fs.writeFileSync(files.claudeHooks, JSON.stringify({
      hooks: ['hook-context', 'hook-precompact', 'hook-prompt', 'hook-tool', 'hook-tool-after'],
    }));
    fs.writeFileSync(files.codexMcp, '[mcp_servers.deja]\ncommand = "/SENTINEL/deja"\n');
    fs.writeFileSync(files.codexHooks, JSON.stringify({ hooks: [
      '/SENTINEL/deja hook-context', '/SENTINEL/deja hook-tool', '/SENTINEL/deja hook-tool-after',
    ] }));
    fs.writeFileSync(files.opencodeMcp, JSON.stringify({
      mcp: { deja: { type: 'local', command: ['/SENTINEL/deja', 'mcp'] } },
    }));
    fs.writeFileSync(files.opencodePlugin,
      '// generated by deja install\nhook-context hook-prompt hook-precompact\n');

    const observed = observeDejaVuTargets({ paths: files });
    assert.deepEqual(observed.claude.direct, { mcp: true, auto: true });
    assert.deepEqual(observed.claude.plugin, { present: false, auto: false });
    assert.equal(observed.claude.projection.mcp.length, 64);
    assert.equal(observed.claude.projection.auto, null,
      'the simplified fixture is detectable but not value-precise hook evidence');
    assert.deepEqual(observed.codex.direct, { mcp: true, auto: true });
    assert.deepEqual(observed.codex.plugin, { present: false, auto: false });
    assert.equal(observed.codex.projection.mcp.length, 64);
    assert.equal(observed.codex.projection.auto, null);
    assert.deepEqual(observed.opencode.direct, { mcp: true, auto: true });
    assert.deepEqual(observed.opencode.plugin, { present: true, auto: true });
    assert.equal(observed.opencode.projection.mcp.length, 64);
    assert.equal(observed.opencode.projection.auto.length, 64);
