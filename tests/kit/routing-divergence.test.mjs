...
// surface says so. A newer default does not establish better task outcomes, so
// assertions pin neutral framing: info rather than warn, and an explicit choice.
...
// Per-token price and end-to-end task cost are distinct. No particular model's
// turn ratio or cache growth law is established by this catalog or its tests.
// Help notes are editorial; the guide points to current evidence and measured tasks.
