...
test('the retired prompts action is absent from help and rejected with all former flags', () => {
  const sb = sandbox();
  const help = ak(['usage', '--help'], sb);
  assert.equal(help.status, 0, help.stderr);
  assert.doesNotMatch(help.stdout, /usage prompts|--deep|--enrich|--draft|--dismiss/);

  const cases = [
    ['usage', 'prompts'],
    ['usage', 'prompts', '--deep'],
    ['usage', 'prompts', '--enrich'],
    ['usage', 'prompts', '--draft', 'card-id'],
    ['usage', 'prompts', '--dismiss', 'card-id'],
  ];
  for (const args of cases) {
    const result = ak(args, sb);
    assert.equal(result.status, 2, `${args.join(' ')}\n${result.stdout}\n${result.stderr}`);
  }
  assert.equal(fs.existsSync(path.join(sb.cfg, 'agentic-kit', 'usage-index.json')), false,
    'a rejected action must not scan transcripts or write a cache');
  fs.rmSync(sb.home, { recursive: true, force: true });
});
