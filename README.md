...
routable host through `ak run`, but it is never a primary host or AQE provider. Existing
`ak dual` is a deprecated Claude+Codex compatibility interface. Provider, model, and billing claims state
...
- **Per-activity routing** — when **both** hosts are enabled (and agentic-qe ≥ 3.13.1), `ak` seeds a
  ...
  **OpenCode is explicit, not seeded.** Enable it with `ak host pick --host claude,opencode`,
  ...
  OpenCode routes are not projected to AQE, do not affect vendor-diversity claims, cannot be a
  primary host, and the deprecated `ak dual` rejects them rather than silently changing its two-host
