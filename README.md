...
| 🎚️ `ruflo-neural-train [args…]` | Thin passthrough to `ruflo neural train` in the current project. Args pass through. The statusline shows `Δ LoRA ✓` as a version-gated capability signal when `@ruvector/ruvllm ≥2.5.6` is installed (F4 fixed in ruflo 3.10.46; real gradient descent verified). |
...
- 📈 **RL** — **live** route Q-learner metrics, shown only once the learner has actually run (`updateCount > 0`): `ε`↓ (exploration), `δ̄`↓ (mean TD error), `|Q|` (distinct task-states — a real count since the encoder fix F3, ruflo #2239, **fixed in 3.10.11, confirmed still correct in 3.10.46**: 6 tasks → 6 distinct Q-states), `upd` (updates). Read fs-only from `.swarm/q-learning-model.json` — which persists across `ruflo route feedback` calls (saveModel, ruflo 3.10.6+); never the broken `route stats` CLI.
...
- **`Δ LoRA ✓`** appears on the SONA line when `@ruvector/ruvllm ≥2.5.6` is installed alongside ruflo (shipped in ruflo 3.10.46). That version replaced the no-op stub with real gradient descent in `processInstantLearning`: computes reward, derives gradient from correction embedding, calls `LoraAdapter.backward()` updating both `loraA` and `loraB`. Verified empirically 2026-06-15: `deltaNorm` moves `0.000000 → 0.001205` after 2 signals. Displayed as a capability signal (not a per-session measurement — runtime microLora weights are in-process only; checkpoint B matrices initialise to zero in standard LoRA). Issue #8 closed.
