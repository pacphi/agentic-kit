...
ak system       machine footprint: install size, runtime, storage, catalog, projects
                [--deep] [--json]
ak maintain     review maintenance findings; plan, apply, undo, or reconcile exact actions
                scan | plan | apply | undo | recover
...
| **dashboard** | Opens the local web dashboard (`127.0.0.1:7431`, localhost-only, never detaches) with five primary areas: **About · Overview · Usage · Observability · System**. Ordinary views remain observation-only. System's **Full scan** remeasures local inventory; Maintenance's **Check providers** uses that saved inventory to recheck versions, ownership, and action eligibility without walking projects. **System → Maintenance** is the sole action surface: it creates server-derived, short-lived plans and requires explicit confirmation plus a one-use capability for one finding at a time. Catalog and Advisory remain measurements. The page is self-contained, offline-first, protected by a per-session token, and never accepts a browser-supplied command or path. Full navigation and security semantics: [Dashboard guide](docs/DASHBOARD.md); provider and recovery limits: [Maintenance runbook](docs/MAINTENANCE.md). **Auto-opens your browser** (`--no-open` for headless/SSH); `--port N` changes the port. Stop with Ctrl-C. (Also available as `ak x dashboard`.) |
...
| **maintain** | Reviews evidence-backed upgrade and cleanup findings, produces ordinary read-only or provider-backed executable plans, then applies exact selected actions with verification and durable receipts. `ak maintain scan` is the provider-only equivalent of **Check providers**; add `--deep` to run the System full measurement first. Planning and actions fail closed while either scan is running. Apply requires the plan ID, digest, action IDs, and `--yes`; guarded undo requires a committed reversible receipt. `recover --receipt ID --yes` only reconciles an interrupted receipt against current provider state—it never retries, applies, or rolls back an uncertain effect. Unsupported hosts and insufficient ownership stay report-only. See [Maintenance](docs/MAINTENANCE.md). |
...
[docs/MAINTENANCE.md](docs/MAINTENANCE.md) — safe scan, plan, apply, undo, and interrupted-receipt
reconciliation, including the exact provider limits.
