...
ak audit hooks  read-only host-neutral hook inventory and proposals
ak hooks        doctor | heal | undo (explicit, receipt-backed hook remediation)
...
| **audit hooks / hooks** | Audits Codex, Claude Code, OpenCode, and admitted external-adapter hook definitions without executing them. `ak hooks doctor --json` preserves the audit JSON contract; `heal --dry-run` emits a deterministic content-bound plan. The first writable recipe is restricted to exact Codex `0.151.0` user-global `SessionEnd` timeout normalization and requires exact action IDs, the plan digest, and confirmation. Every write is backup-first, receipt-backed, verified, and reversible only while later drift is absent. Trust and plugin caches are never changed. See [Hook assurance and healing](docs/HOOKS.md). |
