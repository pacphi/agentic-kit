...
- 🛡️ **Verified security surface** — `ruflo-security-verify` confirms `@claude-flow/security` loads (it absorbed the separate `aidefence` package in 3.28), that prompt-injection defense actually fires, and flags known upstream gaps (the CVE database has no data source; on 3.28.0 `security defend` emits a banner with no verdict — reported distinctly as an upstream regression).
...
| 🛡️ `ruflo-security-verify [--quick]` | Verify `@claude-flow/security` loads (aidefence absorbed into it in 3.28), injection defense fires, scan/secrets run; flags upstream gaps (CVE-DB; the 3.28.0 banner-only `defend` regression) distinctly. |
...
- 🛡️ **security on** — the `@claude-flow/security` package (which absorbed the separate aidefence package in ruflo 3.28; older installs still render `aidefence on`) is loaded (ruflo's native line already shows the `CVE n/m` count, so this signals the *other* half).
