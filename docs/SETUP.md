...
The project guidance result is defined by ownership, not by which initializer ran last:

| Starting state | First `ak setup --project` | Re-run / disable behavior |
| --- | --- | --- |
| No `CLAUDE.md` or `AGENTS.md` | Create only the enabled integrations' bounded project guidance. | A second run is byte-identical when configuration is unchanged. |
| User-authored `CLAUDE.md` | Preserve user text and reconcile agentic-kit sentinels around it. | Replace the owned span in place; disabling removes only that span. |
| User-authored `AGENTS.md`, no `CLAUDE.md` | Preserve `AGENTS.md`; create one line, `@AGENTS.md`, for Claude instead of copying equivalent prose. | The reference stays single and stable. |
| Prior agentic-kit block | Replace its complete sentinel span; collapse duplicate complete copies. | Re-running never stacks another equivalent block. |
| Exact old unsentineled lean stub | Migrate that recognized prior-art shape to current managed guidance. | Future runs use ordinary sentinel ownership. |
| Incomplete sentinel or edited near-match | Preserve as foreign content and report/review the ambiguity. | Never infer deletion authority from a prefix or resemblance. |
...
2026-09-02 reference machine, Claude, Codex and OpenCode machine guidance all reported
`canonical-managed`, while the project's `AGENTS.md` remained untouched.
