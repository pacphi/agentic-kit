...
Theme: these records define **dual-host LLM routing** — how `ak` lets ruflo route each development
activity (architecture, implementation, testing, review, …) to the right host (Claude or Codex) and
model, seeded on detection, tunable by the user, and surfaced across `setup`/`sync`/`status`/`dashboard`.
