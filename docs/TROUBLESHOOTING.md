...
| Hook diagnostics or duplicated lifecycle behavior | Host hook layers compose, and generated/plugin sources may have different canonical owners | Run `ak hooks doctor --host all`; inspect `ak hooks heal --dry-run --json`. Only exact executable actions can be applied, and most generated/project/plugin findings intentionally remain review-only |
| `ak hooks heal` refuses a stale plan or preimage | Runtime version, source bytes, file mode, plan content, or selected action changed after preview | Run a fresh dry-run, review the new diff and impact, then authorize its exact action ID and `--expect-plan` digest. Never reuse an older digest |
| `ak hooks undo` reports postimage drift | A user, host, plugin, or generator changed the target after healing | Preserve the newer file. Compare it with the receipt-backed preview and repair manually; guarded undo will not overwrite later drift |
| `ak hooks doctor` reports an unfinished transaction | A prior heal stopped between journal stages | Run `ak hooks undo --last --dry-run`, inspect the exact receipt/targets, then confirm guarded undo. Do not copy backup files manually |
