...
| Usage → Context says Input only, Partial coverage, Not recorded, or No sessions | The retained session has input evidence but no compatible runtime window, which is normal for current Claude/OpenCode transcripts; an older cache may also predate the v17 context evidence | Let the current schema-20 one-time reparse complete. No sessions means the selected timeframe has no records; missing values render as an em dash. Do not substitute a published model maximum. Codex pressure appears only when its rollout recorded paired input/window evidence |
