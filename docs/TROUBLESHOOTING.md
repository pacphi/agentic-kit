...
| Ruflo refuses every MCP tool call | The project's `.harness/mcp-policy.json` is missing or invalid while `RUFLO_MCP_ENFORCE_POLICY=1` is set — ruflo fails closed rather than enforcing an unreadable policy | Restore a valid, ak-written policy file and run `ak sync`. `ak status` reports this as `mcpGovernance: blocked` and removes the enforcement variable for that project until the file is fixed, so a typo can never lock the project out for good |
