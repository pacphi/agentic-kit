...
| Usage → Context says Partial evidence or unknown | The retained session has input evidence but no compatible runtime window, which is normal for current Claude/OpenCode transcripts; an older cache may also predate the v17 context evidence | Let the current v18 one-time reparse complete. Do not substitute a published model maximum. Codex pressure appears only when its rollout recorded paired input/window evidence |
