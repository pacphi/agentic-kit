...
| `status` shows corrupt/oversized RVF | An interrupted write left store bytes in a `.rvf.lock` (FLVR signature) or a runaway `.rvf`; agentic-qe drops off ruvector with `FsyncFailed` and does **not** self-heal this | `ak sync` quarantines it — the store is a derived cache, rebuilt from `memory.db` |
| Statusline footer (🧠/🛡/🎓 lines) disappeared | `ruflo init`/upgrade regenerated `statusline.cjs` | `ak sync` re-injects it |
| Too many `⚙` daemons / stale daemons | One daemon per active project is normal (local-only workers, $0). Stale = workspace deleted or past the 12h TTL | `ak x daemon-gc --kill`; `sync` also reaps. AI workers only ever run with `RUFLO_DAEMON_AI_WORKERS=1`, capped by `ruflo daemon budget show` |
