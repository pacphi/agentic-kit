...
| OpenCode inventory says `partial` with `invalid-model-id` | An older Agentic Kit parser rejected current OpenRouter `~publisher/model` selectors, or a newer OpenCode release emitted a genuinely unsupported selector | Upgrade Agentic Kit and run `ak models refresh` again. Current `~` selectors and verbose metadata are supported; if the diagnostic remains, inspect `ak models status --json` and report the redacted selector grammar rather than treating the partial list as complete |
...
| Models shows `unknown` instead of yes/no | No accepted source established that independent fact; catalogue rows commonly prove only discovery | Expand the cell for its field-specific reason. Refresh the relevant source, but do not infer entitlement from catalogue visibility, successful use from configuration, serving provider from a model name, or quality from lifecycle metadata |
