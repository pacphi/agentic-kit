...
function inspectPlugin(ref, cacheDir) {
  ...
  if (!parsed) return { ref, version: null, root: null, hookFiles: [], issues: [`invalid plugin reference "${ref}"`] };
  const base = path.join(cacheDir, parsed.marketplace, parsed.plugin);
  const version = newestVersionDir(base);
  if (!version) {
    return { ref, version: null, root: null, hookFiles: [], issues: [`${ref}: enabled but no cached version is installed`] };
  }
  ...
  if (!manifestFile) {
    return {
      ref, version, root, hookFiles: [],
      issues: [`${ref}: cached generation ${version} has no supported plugin manifest`],
    };
  }
  const manifest = readJson(manifestFile);
  if (manifest.error) {
    return { ref, version, root, hookFiles: [], issues: [`${manifestFile}: ${manifest.error}`] };
  }
  ...
  return { ref, version, root, hookFiles, issues };
}
