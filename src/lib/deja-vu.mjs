...
export function buildDejaVuUninstallCommand(host, mode) {
  return deepFreeze({
    command: DEJA_VU_BIN,
    args: ['uninstall', dejaVuTarget(host, mode), '--no-guidance', '--no-index'],
  });
}
...
function jsonObject(file) {
  ...
  if (text === null) return { present: null, value: null };
  try {
    const value = JSON.parse(text);
    return isRecord(value) ? { present: true, value } : { present: null, value: null };
  } catch {
...
function collectStrings(value, out = [], depth = 0) {
  if (depth > 8 || out.length > 4096) return out;
  if (typeof value === 'string') { out.push(value); return out; }
  if (Array.isArray(value)) {
    for (const child of value) collectStrings(child, out, depth + 1);
...
function hookProjection(file, expected) {
  const parsed = jsonObject(file);
  if (parsed.present !== true) return { complete: parsed.present, hash: null };
  const projection = [];
  for (const [event, subcommand] of expected) {
    const eventEntries = parsed.value.hooks?.[event];
    if (!Array.isArray(eventEntries)) continue;
    for (const entry of eventEntries) {
      if (!isRecord(entry) || !Array.isArray(entry.hooks)) continue;
      for (const hook of entry.hooks) {
        if (!isRecord(hook) || typeof hook.command !== 'string'
          || !hook.command.trim().endsWith(subcommand)) continue;
        projection.push({
          event,
  ...
  const covered = new Set(projection.map(({ event }) => event));
  return {
    complete: expected.every(([event]) => covered.has(event)),
    hash: projection.length > 0 ? projectionHash(projection) : null,
  };
}

function hookSet(file, expected) {
  const parsed = jsonObject(file);
  if (parsed.present !== true) return parsed.present;
  const strings = collectStrings(parsed.value);
  return expected.every((command) => strings.some((value) => value.trim().endsWith(command)));
}

function claudePluginState(registryFile, settingsFile, cwd) {
  const registry = jsonObject(registryFile);
  const settings = jsonObject(settingsFile);
  if (registry.present === null || settings.present === null) {
    return { present: null, auto: null };
  }
  if (registry.present !== true || settings.present !== true) {
    return { present: false, auto: false };
  }
  const enabled = settings.value.enabledPlugins;
  const plugins = registry.value.plugins;
  if (!isRecord(enabled) || !isRecord(plugins)) return { present: null, auto: null };
  const resolvedCwd = path.resolve(cwd);
  const active = Object.entries(enabled).some(([ref, value]) => {
    if (value !== true || !ref.startsWith('deja-vu@')) return false;
    const installs = plugins[ref];
    return Array.isArray(installs) && installs.some((entry) => {
      if (!isRecord(entry)) return false;
      if (entry.scope === undefined || entry.scope === 'user') return true;
      return (entry.scope === 'project' || entry.scope === 'local')
        && typeof entry.projectPath === 'string'
        && path.resolve(entry.projectPath) === resolvedCwd;
    });
  });
  // The v0.19 Claude plugin declares MCP plus all five automatic hook events.
  return { present: active, auto: active };
}

function tomlSections(source, headerPattern) {
  const matches = [];
  const table = /^\[([^\r\n]+)\]\s*$/gm;
  let match;
  while ((match = table.exec(source)) !== null) {
    const next = source.slice(table.lastIndex).search(/^\[/m);
    const body = source.slice(table.lastIndex,
      next < 0 ? source.length : table.lastIndex + next);
    if (headerPattern.test(match[1])) matches.push({ header: match[1], body });
    headerPattern.lastIndex = 0;
  }
  return matches;
}

function codexPluginState(configFile) {
  const source = boundedText(configFile);
  if (source === null) return { present: null, auto: null };
  if (source === '') return { present: false, auto: false };
  const plugin = tomlSections(source, /^plugins\."deja-vu@[^"\r\n]+"$/)
    .some(({ body }) => /^\s*enabled\s*=\s*true(?:\s*#.*)?$/m.test(body));
  if (!plugin) return { present: false, auto: false };

  // Codex records trust per hook. Presence of a trusted_hash is evidence that
  // the host saw and accepted that hook, not proof that the hash matches a
  // reproducible digest (v0.19 explicitly documents that limitation).
  const trustedEvents = new Set();
  for (const { header, body } of tomlSections(source,
    /^hooks\.state\."deja-vu@[^"\r\n]+:.*hooks\/hooks\.json:(?:session_start|user_prompt_submit):\d+:\d+"$/)) {
    if (/^\s*enabled\s*=\s*false(?:\s*#.*)?$/m.test(body)) continue;
    if (!/^\s*trusted_hash\s*=\s*"sha256:[^"]+"\s*$/m.test(body)) continue;
    if (header.includes(':session_start:')) trustedEvents.add('session_start');
    if (header.includes(':user_prompt_submit:')) trustedEvents.add('user_prompt_submit');
  }
  return {
    present: true,
    auto: trustedEvents.has('session_start') && trustedEvents.has('user_prompt_submit'),
  };
}

/**
 * Read only the six direct surfaces Kit may manage. The facts contain booleans
...
export function observeDejaVuTargets(options = {}) {
  ...
  const claudeMcp = jsonObject(files.claudeMcp);
  const claudeMcpState = claudeMcp.present === true
    ? Object.hasOwn(claudeMcp.value.mcpServers ?? {}, 'deja') : claudeMcp.present;
  const claudeMcpHash = claudeMcpState === true
    ? projectionHash(claudeMcp.value.mcpServers.deja) : null;
  const claudeAutoProjection = hookProjection(files.claudeHooks, [
  ...
  const codexText = boundedText(files.codexMcp);
  const codexMcpState = codexText === null ? null
    : codexText === '' ? false : /^\s*\[mcp_servers\.deja\]\s*$/m.test(codexText);
  const codexMcpSection = codexMcpState === true
    ? tomlSections(codexText, /^mcp_servers\.deja$/)[0] : null;
  const codexMcpHash = codexMcpSection
    ? projectionHash(codexMcpSection.body.replaceAll('\r\n', '\n').trim()) : null;
  const codexAutoProjection = hookProjection(files.codexHooks, [
    ['SessionStart', 'hook-context'],
    ['PreToolUse', 'hook-tool'],
    ['PostToolUse', 'hook-tool-after'],
  ]);
  let opencodeText = boundedText(files.opencodeMcp);
  if (opencodeText === '' && files.opencodeMcp.endsWith('.json')) {
    opencodeText = boundedText(`${files.opencodeMcp}c`);
  }
  const opencodeMcpState = opencodeText === null ? null
    : opencodeText === '' ? false
      : /["']mcp["']\s*:/.test(opencodeText) && /["']deja["']\s*:/.test(opencodeText);
  let opencodeMcpHash = null;
  if (opencodeMcpState === true) {
    try {
      const parsed = JSON.parse(opencodeText);
      opencodeMcpHash = isRecord(parsed?.mcp?.deja)
        ? projectionHash(parsed.mcp.deja) : projectionHash(opencodeText);
    } catch {
      // JSONC has no parser in this zero-runtime-dependency CLI. Hashing the
      // bounded config is conservative: unrelated edits refuse teardown.
      opencodeMcpHash = projectionHash(opencodeText);
    }
  }
  const opencodePlugin = boundedText(files.opencodePlugin);
  const opencodeAuto = opencodePlugin === null ? null : opencodePlugin === '' ? false
    : opencodePlugin.includes('generated by deja install')
      && ['hook-context', 'hook-prompt', 'hook-precompact']
        .every((command) => opencodePlugin.includes(command));
  const claudePlugin = claudePluginState(files.claudePlugins, files.claudeHooks,
    options.cwd ?? process.cwd());
  const codexPlugin = codexPluginState(files.codexMcp);
  return deepFreeze({
    claude: {
      direct: {
        mcp: claudeMcpState,
        auto: hookSet(files.claudeHooks, [
          'hook-context', 'hook-precompact', 'hook-prompt', 'hook-tool', 'hook-tool-after',
        ]),
      },
      projection: { mcp: claudeMcpHash, auto: claudeAutoProjection.hash },
      plugin: claudePlugin,
    },
    codex: {
      direct: {
        mcp: codexMcpState,
        auto: hookSet(files.codexHooks, ['hook-context', 'hook-tool', 'hook-tool-after']),
      },
      projection: { mcp: codexMcpHash, auto: codexAutoProjection.hash },
      plugin: codexPlugin,
    },
    opencode: {
      direct: { mcp: opencodeMcpState, auto: opencodeAuto },
      projection: {
        mcp: opencodeMcpHash,
        auto: opencodeAuto === true ? projectionHash(opencodePlugin) : null,
      },
      plugin: { present: opencodeAuto === true, auto: opencodeAuto === true },
    },
  });
}
